<template>
    <view class="more">
		...
		<view class="title2">冥想</view>
		<scroll-view class="scroll-view_H" scroll-x style="width: 100%" scroll-with-animation>
			<view class="title_wrap">
				<view :class="[item==select_title ? 'title_item_select1' : 'title_item_unselect','title_item']" v-for="(item,index) in titles" :key="index"  @tap="selectTitle(item)">
					<text>{{item}}</text>
				</view>
			</view>
		</scroll-view>
		<scroll-view scroll-y class="story_wrap">
            <view v-for="(item,index) in meditateList" :key="index" class="story_item" @tap="goplayer(item)">
	...
	export default {
		...
		computed:{
			...mapState({
				meditateList(state){
					var list=[];
					for(let i in state.meditateList){
						list.push(state.meditateList[i]);
						}
					return list;
				}
			})
...
<style>
	...
	.title_item_select1{
		background: linear-gradient(left, #FCBABA 0%,#B1E0E2 100%);
	}
	.story_wrap{
		height: 1100upx;
		display: flex;
